...
export class BalanceSnapshotsService {
  ...
  async update(
    ...
    if (updateBalanceSnapshotDto.cashBalance !== undefined) {
      snapshot.cashBalance = String(updateBalanceSnapshotDto.cashBalance);
    }

    if (updateBalanceSnapshotDto.eMoneyBalance !== undefined) {
      snapshot.eMoneyBalance = String(updateBalanceSnapshotDto.eMoneyBalance);
    }

    if (updateBalanceSnapshotDto.recordedAt !== undefined) {
      snapshot.recordedAt = new Date(updateBalanceSnapshotDto.recordedAt);
    }
